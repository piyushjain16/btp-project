...
logging.basicConfig(filename="gamma_DCE_sigmoid_gamma_beta_map_0.4_1.4.log", filemode="a", format="%(levelname)s: %(message)s",level=logging.INFO)
...
def lowlight(image_path, label_path,gamma_net):
...
def test_lowlightimage(gamma_net,config,epoch):
	with torch.no_grad():
		sum_psnr = 0
		sum_ssim = 0
		test_size = 0
		...
		for image in test_list:
			...
			psnr, ssim = lowlight(image, image_label,gamma_net)
			test_size += 1
			sum_psnr += psnr
			sum_ssim += ssim
...
def train(config):
	...
	for epoch in range(config.num_epochs):
		for iteration, img_lowlight in enumerate(train_loader):
			...
			enhanced_image,gamma ,beta  = gamma_net(img_lowlight)
			...
			# experiment 2 activation as sigmoid and diffrent weights to losses 
			Loss_TV = 300*(L_TV(gamma) + L_TV(beta))
			
			loss_spa = 2*torch.mean(L_spa(enhanced_image, img_lowlight))

			loss_col = 7*torch.mean(L_color(enhanced_image))
		...
		test_lowlightimage(gamma_net,config,epoch) 		
...
if __name__ == "__main__":
	...
	parser.add_argument('--snapshots_folder', type=str, default="E:/python/Zero-DCE-master/Zero-DCE_code/snapshot_gamma_with_gamma_beta_map_0.4_1.4/")
